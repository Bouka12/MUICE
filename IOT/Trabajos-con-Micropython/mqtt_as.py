...
# Nomre y Apellido = Mabrouka Salmi

#___________________________________________#

import gc   # Recolector de basura para gestión de memoria
import usocket as socket    # import Módulo socket de MicroPython
import ustruct as struct    # import Módulo struct de MicroPython

gc.collect()    # Activar recolector de basura
from ubinascii import hexlify    # Módulo binascii de MicroPython para conversión a hexadecimal
import uasyncio as asyncio  # Módulo de E/S asíncrona para MicroPython

gc.collect()     # Otra ronda de recolección de basura
from utime import ticks_ms, ticks_diff   # Funciones del módulo de tiempo de MicroPython
from uerrno import EINPROGRESS, ETIMEDOUT   # Códigos de error del módulo errno de MicroPython


gc.collect()     # Recolector de basura
from micropython import const   # Módulo de MicroPython para definición de constantes
from machine import unique_id   # Módulo de MicroPython para IDs de máquina únicos
import network   # Módulo de red de MicroPython para configuraciones de red

gc.collect()    # Recolector de basura   
from sys import platform    # Módulo de MicroPython para información de plataforma


VERSION = (0, 7, 0)      # Versión del módulo MQTT

# Default short delay for good SynCom throughput (avoid sleep(0) with SynCom).
_DEFAULT_MS = const(20)      # Constante de retardo por defecto
...
ESP32 = platform == "esp32"     # Comprobación de si la plataforma es ESP32
RP2 = platform == "rp2"      # Comprobación de si la plataforma es RP2
if ESP32:
    ...
    BUSY_ERRORS = [EINPROGRESS, ETIMEDOUT, -110]    # Lista de errores ocupados para RP2
else:
    BUSY_ERRORS = [EINPROGRESS, ETIMEDOUT]      # Lista de errores ocupados para otras plataformas

ESP8266 = platform == "esp8266"     # Comprobación de si la plataforma es ESP8266
PYBOARD = platform == "pyboard"     # Comprobación de si la plataforma es Pyboard

# Default "do little" coro for optional user replacement
async def eliza(*_):  # e.g. via set_wifi_handler(coro): see test program
    await asyncio.sleep_ms(_DEFAULT_MS)     # Sueño por defecto para el coro


class MsgQueue:
    def __init__(self, size):
        self._q = [0 for _ in range(max(size, 4))]      # Inicialización de la cola de mensajes
        self._size = size    # Tamaño de la cola de mensajes
        self._wi = 0        # Índice de escritura para la cola
        self._ri = 0    # Índice de lectura para la cola
        self._evt = asyncio.Event() # Bandera de evento para eventos asíncronos
        self.discards = 0   # Contador de mensajes descartados

    def put(self, *v):
        self._q[self._wi] = v   # Colocar mensaje en la cola
        self._evt.set() # Establecer bandera de evento
        self._wi = (self._wi + 1) % self._size # Incrementar índice de escritura
        if self._wi == self._ri:  # Would indicate empty
            self._ri = (self._ri + 1) % self._size  # Discard a message
            self.discards += 1   # Incrementar contador de descartes

    def __aiter__(self):
        return self

    async def __anext__(self):
        if self._ri == self._wi:  # # Comprobar si la cola está vacía
            self._evt.clear()       # Limpiar bandera de evento
            await self._evt.wait()  # Esperar un evento
        r = self._q[self._ri]   # Obtener el mensaje de la cola
        self._ri = (self._ri + 1) % self._size   # Incrementar índice de lectura
        return r    # Devolver el mensaje


# Diccionario de configuración con ajustes por defecto
config = {
    "client_id": hexlify(unique_id()),  # ID de cliente basado en el ID único de la máquina
    "server": None, # Servidor del broker MQTT
    "port": 0,  # Puerto para la conexión
    "user": "", # Usuario para autenticación
    "password": "", # Contraseña para autenticación
    "keepalive": 60,    # Tiempo de keep-alive
    "ping_interval": 0, # Intervalo de ping
    "ssl": False,    # Uso de SSL/TLS
    "ssl_params": {},   # Parámetros SSL
    "response_time": 10,    # Tiempo de respuesta
    "clean_init": True, # Inicialización limpia
    "clean": True,   # Sesión limpia
    "max_repubs": 4,    # Número máximo de republicaciones
    "will": None,   # Testamento
    "subs_cb": lambda *_: None, # Callback de suscripción
    "wifi_coro": eliza, # Coro de WiFi
    "connect_coro": eliza,  # Coro de conexión
    "ssid": None,   # SSID de red WiFi
    "wifi_pw": None,    # Contraseña de red WiFi
    "queue_len": 0,  # Longitud de la cola
}
...
# Generador de PID para mensajes MQTT
def pid_gen():
    pid = 0
    while True:
...
# Verifica el nivel de calidad de servicio (QoS)
def qos_check(qos):
...
class MQTT_base:
    # Class variables
    REPUB_COUNT = 0  # TEST # Counter for republishing
    DEBUG = False   # Debug mode

    def __init__(self, config):
        # Initialize MQTT base with configuration settings
        # Event handling setup based on the queue length in the config
        ...
        # Configuración del mensaje de voluntad
        will = config["will"]
        if will is None:
        ...
        # Callbacks and coros # Configuraciones de devolución de llamada y corutinas basadas en manejo de eventos
        if self._events:
            self.up = asyncio.Event()
            self.down = asyncio.Event()
            self.queue = MsgQueue(config["queue_len"])   # Cola de mensajes para eventos
        else:  # Callbacks
            # Devoluciones de llamada para suscripción MQTT y conexión WiFi
        ...
        # Inicializar generador de PID y seguimiento de PID para PUBACK/SUBACK
        self.newpid = pid_gen()
        self.rcv_pids = set()  # PUBACK and SUBACK pids awaiting ACK response
        self.last_rx = ticks_ms()  # Time of last communication from broker
        self.lock = asyncio.Lock()  # Bloqueo para sincronización

    # Método para configurar el mensaje de voluntad
    ...
    def dprint(self, msg, *args):
        if self.DEBUG:
            print(msg % args)
    # Método de cálculo de tiempo de espera
    def _timeout(self, t):
        return ticks_diff(ticks_ms(), t) > self._response_time
    # Método de lectura de socket asíncrono
    async def _as_read(self, n, sock=None):  # OSError caught by superclass
        ...
        return data
    # Método de escritura de socket asíncrono
    async def _as_write(self, bytes_wr, length=0, sock=None):
        ...
        while bytes_wr:
            ...
            if n:
                ...
                bytes_wr = bytes_wr[n:]
            await asyncio.sleep_ms(_SOCKET_POLL_DELAY)
    # Método para enviar una cadena de forma asíncrona
    async def _send_str(self, s):
        await self._as_write(struct.pack("!H", len(s)))
        await self._as_write(s)
    # Método asíncrono para recibir longitud
    async def _recv_len(self):
        ...
        while 1:
            ...
            if not b & 0x80:
                return n
            sh += 7
    # Método asíncrono para conectar al broker MQTT
    ...
    # Método asíncrono para enviar un PING
    ...
    async def wan_ok(
        ...
        try:
            ...
            s.close()
        return False
     # Método para probar la conectividad con el broker
    async def broker_up(self):  # Test broker connectivity
        ...
        while not self._timeout(t):
            ...
            if ticks_diff(self.last_rx, tlast) > 0:  # Response received
                return True
        return False
    # Método para desconectarse del broker
    ...
    def _close(self):
        if self._sock is not None:
            self._sock.close()
     # Método para cerrar la conexión (API)
    ...
    # Método para esperar respuesta PID
    ...
    async def publish(self, topic, msg, retain, qos):
        ...
        while 1:  # Await PUBACK, republish on timeout
            ...
            count += 1
            self.REPUB_COUNT += 1
    # Método para publicar mensajes internamente
