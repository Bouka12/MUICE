...
nodes <- read.csv("mi-nodos.csv", header = T)
links <- read.csv("mi-enlaces.csv", header = T)
library(igraph)
# Load the necessary library
library(igraph)
library(dplyr)

# Preprocess 'nodes' data frame
nodes <- read.table(text = as.character(nodes$Id.Label), sep = ";", col.names = c("id", "label"))

# Preprocess 'links' data frame
links <- read.table(text = as.character(links$Source.Target.Weight.Type), sep = ";", 
                    col.names = c("from", "to", "weight", "type"))
...
g4c <- induced_subgraph(net.sym, kcore==12)
...
# Calculate the number of unique k-core levels
unique_cores <- unique(kcore)
number_of_kcores <- length(unique_cores)

# Find the maximum k-core value
max_kcore_value <- max(kcore)

# Count the number of vertices in the maximum k-core
max_kcore_count <- sum(kcore == max_kcore_value)

# Printing results
cat("Number of distinct k-cores: ", number_of_kcores, "\n")
cat("Highest k-core degree (k): ", max_kcore_value, "\n")
cat("Number of elements in the highest k-core: ", max_kcore_count, "\n")
